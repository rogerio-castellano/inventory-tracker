...
func NewRouter() http.Handler {
	...
	loginRateLimiter := RedisRateLimitMiddleware("login", 5, time.Minute)
	r.With(loginRateLimiter).Post("/login", handlers.LoginHandler)
	...
	refreshRateLimiter := RedisRateLimitMiddleware("refresh", 10, time.Minute)
	r.With(refreshRateLimiter).Post("/refresh", handlers.RefreshHandler)
	...
	r.Route("/admin", func(r chi.Router) {
		...
		adminRateLimiter := RedisRateLimitMiddleware("admin", 3, time.Minute)
		r.With(adminRateLimiter).Post("/users/{username}/tokens", handlers.AdminImpersonateUserHandler)
	})
